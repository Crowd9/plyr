typeof navigator === "object" && (function () {
	...
	*/
	...
	var INCORRECT_ITERATION$3 = FORCED$7 || !checkCorrectnessOfIteration$1(function (iterable) {
	...
	function () {
	  ...
	  function RangeTouch(target, options) {
	    ...
	    if (is$2.element(target)) {
	      ...
	      this.element = target;
	    } else if (is$2.string(target)) {
	      // A CSS Selector is passed, fetch it from the DOM
	      this.element = document.querySelector(target);
	    }

	    if (!is$2.element(this.element) || !is$2.empty(this.element.rangeTouch)) {
	      return;
	    }

	    this.config = Object.assign({}, defaults$2, options);
	    this.init();
	  }
	  ...
	  _createClass(RangeTouch, [{
	    ...
	    value: function get(event) {
	      if (!RangeTouch.enabled || !is$2.event(event)) {
	        return null;
	      }
	    ...
	    value: function set(event) {
	      if (!RangeTouch.enabled || !is$2.event(event) || event.target.disabled) {
	        return;
	    ...
	    value: function setup(target) {
	      ...
	      if (is$2.empty(target) || is$2.string(target)) {
	        targets = Array.from(document.querySelectorAll(is$2.string(target) ? target : 'input[type="range"]'));
	      } else if (is$2.element(target)) {
	        targets = [target];
	      } else if (is$2.nodeList(target)) {
	        targets = Array.from(target);
	      } else if (is$2.array(target)) {
	        targets = target.filter(is$2.element);
	      }
	      ...
	      if (is$2.string(target) && config.watch) {
	        // Create an observer instance
	        var observer = new MutationObserver(function (mutations) {
	          Array.from(mutations).forEach(function (mutation) {
	            Array.from(mutation.addedNodes).forEach(function (node) {
	              if (!is$2.element(node) || !matches$1(node, target)) {
	                return;
	              } // eslint-disable-next-line no-unused-vars
